...
public class theRobot extends JFrame {
    ...
    public theRobot(String _manual, int _decisionDelay) {
        ...
        int height = 500;
        int bar = 20;
        setSize(width*2+20,height+bar);
        getContentPane().setBackground(bkgroundColor);
        setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
        setBounds(0, 0, width*2+20, height+bar);
    ...
    void valueIteration() {
        // you'll need to write the rest of this code to do value iteration
        // the current code just initializes the value of being each state Vs[x][y], and sends these to myMaps for drawing to the GUI
        int x, y;
        
        // initialize the values to 0 for each state (x, y)
        ...
        myMaps.updateValues(Vs);  // send the updated values of being in each state
    }
    ...
    void updateProbabilities(int action, String sonars) {
        ...
        for(i = 0; i < mundo.height; i++){
            for(j = 0; j < mundo.width; j++){
                if(mundo.grid[i][j] == 0){
                    possibleStates++;
                }
            }
    ...
    // TODO: this is the function you'll need to write to make the robot move using your AI
    int automaticAction() {
